...
import { InventoryItem, EstimateItem, CalculationResult } from '@/types';
...
import { MinusCircle, Plus, Trash2 } from 'lucide-react-native';
...
export default function EstimatorScreen() {
    ...
    const [welders, setWelders] = useState('0');
    ...
    useEffect(() => {
        if (selectedProjectId && projects.length) {
            ...
            if (p && p.internal_estimate) {
                ...
                if (est.welders) setWelders(String(est.welders));
                if (est.helpers) setHelpers(String(est.helpers));
            } else {
                setItems([]);
                setLaborDays('1');
            }
    ...
    }, [items, laborDays, welders, helpers, settings]);

    // Dynamic Inventory Filtering
    useEffect(() => {
        if (invType && inventory.length) {
            // Filter inventory by type to get dims
        ...
        <DeepSpaceBackground>
            <ScrollView className="flex-1 p-4">
                ...
                <GlassCard className="mb-4">
                    <Text className="text-slate-400 text-xs uppercase mb-1">Select Client</Text>
                    <View className="bg-slate-900 border border-slate-700 rounded-lg mb-3">
                        <Picker
                    ...
                    <Text className="text-slate-400 text-xs uppercase mb-1">Select Project</Text>
                    <View className="bg-slate-900 border border-slate-700 rounded-lg">
                        <Picker
                    ...
                    <>
                        ...
                        <GlassCard className="mb-4">
                            ...
                            <View className="flex-row gap-2 mb-2">
                                <View className="flex-1 bg-slate-900 rounded-lg">
                                ...
                                <View className="flex-1 bg-slate-900 rounded-lg">
                                    <Picker selectedValue={invDim} onValueChange={setInvDim} style={{ color: 'white' }}>
                                        <Picker.Item label="Dimensions..." value="" />
                            ...
                            {items.map((item, idx) => (
                                <GlassCard key={idx} className="mb-2 p-3 flex-row justify-between items-center">
                                    <View className="flex-1">
                                        ...
                                        <Text className="text-slate-400 text-xs">{item.Qty} {item.Unit} x ₹{item['Base Rate']}</Text>
                                    </View>
                        ...
                        {/* Labor Config */}
                        <GlassCard className="mb-4">
                            <Text className="text-white font-bold mb-2">Labor</Text>
                            <View className="flex-row gap-2">
                                <View className="flex-1">
                                    <Text className="text-slate-400 text-xs">Days</Text>
                                    <TextInput className="bg-slate-900 text-white p-2 rounded border border-slate-700 mt-1" value={laborDays} onChangeText={setLaborDays} keyboardType="numeric" />
                                </View>
                                <View className="flex-1">
                                    <Text className="text-slate-400 text-xs">Welders</Text>
                                    <TextInput className="bg-slate-900 text-white p-2 rounded border border-slate-700 mt-1" value={welders} onChangeText={setWelders} keyboardType="numeric" />
                                </View>
                                <View className="flex-1">
                                    <Text className="text-slate-400 text-xs">Helpers</Text>
                                    <TextInput className="bg-slate-900 text-white p-2 rounded border border-slate-700 mt-1" value={helpers} onChangeText={setHelpers} keyboardType="numeric" />
                                </View>
                            </View>
                        </GlassCard>
                        ...
                        {result && (
                            <GlassCard className="mb-6 bg-blue-900/20">
                                ...
                                <View className="flex-row justify-between">
                                    <Text className="text-slate-400">Profit</Text>
